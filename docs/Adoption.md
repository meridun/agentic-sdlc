...
| Azure DevOps, several repos per deliverable | [`ado-feature`](../sdlc/bindings/ado-feature/BINDING.md) | write (or port) an `sdlc.ps1` against its operation table; tag families per its § 3 |
| Azure DevOps, one repo, a PBI is shippable | [`ado-pbi`](../sdlc/bindings/ado-pbi/BINDING.md) — *proposed, validate on first use* | as `ado-feature`, minus the Feature level |
...
| `TOKEN_TOOL` | shell-output compactor — either an explicit prefix on every command, or a transparent shell wrapper (see `agents/sdlc-worker.md` for the two binding modes) | `tok` |
...
[Composability.md](Composability.md#the-conformance-profile)). Worked examples:
[profiles/gh-issue.example.md](profiles/gh-issue.example.md) (the minimal single-repo GitHub case
with the reference CLI), [profiles/ado-feature.example.md](profiles/ado-feature.example.md) (a
multi-repo Azure DevOps read-only consumer), and
[profiles/ado-pbi.example.md](profiles/ado-pbi.example.md) (the proposed single-repo ADO shape).
...
Upstream sync is manual (see [Composability.md](Composability.md#distribution-model-hybrid-fork-per-project)).
